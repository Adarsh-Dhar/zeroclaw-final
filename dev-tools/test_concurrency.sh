...
# Concurrency Test for ZeroClaw - Test #1 (Highest Priority)
...
echo "=== Concurrency Test for ZeroClaw ==="
...
# NOTE: We're using the actual config directory directly, no modifications needed
echo "Using actual configuration without modifications"
...
/opt/homebrew/bin/zeroclaw daemon --config-dir "$CONFIG_DIR" > /tmp/test-daemon-concurrency.log 2>&1 &
DAEMON_PID=$!
echo "Daemon started with PID: $DAEMON_PID"
echo "Waiting 15 seconds for daemon to fully initialize and load SOPs..."
sleep 15
...
echo "Will trigger 5 concurrent SOP runs to test claim-release mechanism"
echo ""

# Array to store run IDs
declare -a RUN_IDS

# Use role_audit as test SOP (we know it has manual trigger)
TEST_SOP="role_audit"
echo "Using role_audit SOP for testing (has manual trigger)"

# Verify it's loaded
echo "Verifying SOP is loaded..."
SOPS_LIST=$(curl -s http://127.0.0.1:42617/api/sops \
  -H "Authorization: Bearer $TOKEN")
echo "Available SOPs: $SOPS_LIST"

SOP_COUNT=$(echo "$SOPS_LIST" | jq '.sops | length' 2>/dev/null || echo "0")
echo "SOP count: $SOP_COUNT"

if [ "$SOP_COUNT" -eq 0 ]; then
    echo "❌ ERROR: No SOPs loaded by daemon. Waiting additional 10 seconds..."
    sleep 10
    SOPS_LIST=$(curl -s http://127.0.0.1:42617/api/sops \
      -H "Authorization: Bearer $TOKEN")
    echo "Retry available SOPs: $SOPS_LIST"
    SOP_COUNT=$(echo "$SOPS_LIST" | jq '.sops | length' 2>/dev/null || echo "0")
    echo "Retry SOP count: $SOP_COUNT"
    
    if [ "$SOP_COUNT" -eq 0 ]; then
        echo "❌ ERROR: Still no SOPs loaded. This indicates a daemon configuration issue."
        echo "The daemon may not be loading SOPs from the sops_dir correctly."
        echo "Test cannot proceed without loaded SOPs."
        kill $DAEMON_PID 2>/dev/null || true
        exit 1
    fi
fi

# Trigger 5 concurrent runs
for i in {1..5}; do
    echo "Triggering run #$i for $TEST_SOP..."
    RESPONSE=$(curl -s -X POST http://127.0.0.1:42617/api/sops/$TEST_SOP/run \
...
RESPONSE_NEW=$(curl -s -X POST http://127.0.0.1:42617/api/sops/$TEST_SOP/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{}')
...
tail -200 /tmp/test-daemon-concurrency.log | grep -i "claim\|release\|finish_run" || echo "No claim/release logs found"
...
# No config restoration needed since we used the actual config directory
echo "No config restoration needed"
...
echo ""
echo "Test completed. Logs saved to: /tmp/test-daemon-concurrency.log"
