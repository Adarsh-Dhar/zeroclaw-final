...
1. **Poll Subscribe_Channel for recent messages** — Retrieve the last 20 messages from Subscribe_Channel via the Proxy.
   - tools: http_request
   ...
   **Filter by timestamp:** Only process messages sent within the last 5 minutes (300 seconds). Calculate the cutoff timestamp as `current_time - 300 seconds`. Skip any message where `message.timestamp < cutoff_timestamp`.
...
2. **Process messages (newest-first)** — Iterate through messages, filter bots, detect subscribe commands, and invoke Onboarding SKILL.
   - tools: http_request
   ...
   **Check for Standalone `subscribe` Command:**
   ...
   - If the regex matches: this is a qualifying subscribe command. Continue to tier extraction.
